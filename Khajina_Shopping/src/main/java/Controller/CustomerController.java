...
//			s.setPassword(request.getParameter("password"));
			...
			if(flag==true) {
				if(np.equals(cnp)) {
					CustomerDao.updatepassword(np,id);
					response.sendRedirect("customer-home.jsp");
				}
				else {
					request.setAttribute("msg1", "new  password and  confirm new password not matched");
					request.getRequestDispatcher("customer_cgp.jsp").forward(request, response);
				}
			}
			else {
		...
		else if (action.equalsIgnoreCase("sent otp")) {
			String email = request.getParameter("email");
			Random r = new Random();
			...
			if(otp1 == otp2) {
				request.setAttribute("email", email);
				request.getRequestDispatcher("customer-newpassword.jsp").forward(request, response);
			}
			else {
				request.setAttribute("email", email);
